...
class ConvNet(object):
    ...
    def model_2(self, X, hidden_size):
        ...
        convLayer1 = tf.layers.max_pooling2d(convLayer1, pool_size=2, 
                                             strides=2, padding="SAME")

        convLayer2 = tf.layers.conv2d(convLayer1, filters=40, kernel_size=5, 
                                      padding="SAME", activation=tf.nn.sigmoid)
        convLayer2 = tf.layers.max_pooling2d(convLayer2, pool_size=2, 
                                             strides=2, padding="SAME")

        convLayer2 = tf.contrib.layers.flatten(convLayer2)
        num_output = 10
        fullyConLayer = tf.layers.dense(convLayer2, hidden_size, activation=tf.nn.sigmoid)
    ...
    def model_4(self, X, hidden_size, decay):
        ...
        convLayer2 = tf.contrib.layers.flatten(convLayer2)
        num_output = 10
        fullyConLayer1 = tf.layers.dense(convLayer2, hidden_size, activation=tf.nn.sigmoid)
    ...
    def model_5(self, X, hidden_size, is_train):
        ...
        dropRate = 0.5
        dropout1 = tf.layers.dropout(fullyConLayer2, dropRate, training=is_train)

        logits = tf.layers.dense(dropout1, num_output)
        return logits
